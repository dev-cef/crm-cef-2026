...
import { criarCategoria, excluirCategoria } from "@/app/(app)/biblioteca/actions";
...
export default function CategoriasClient({ categorias }: { categorias: Categoria[] }) {
  ...
  async function handleCriar() {
    if (!nome.trim()) return;
    setLoading(true);
    const result = await criarCategoria(nome, descricao);
    setLoading(false);
    if (result.ok) {
  ...
  async function handleExcluir(id: string, nomeCat: string) {
  ...
  return (
    <div className="space-y-6 p-6 max-w-xl">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold">Categorias da Biblioteca</h1>
      </div>
      ...
      <div className="rounded-xl border bg-card p-5 space-y-4">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <Input id="nome" value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex: Montanhismo e Escalada" />
          </div>
          <div className="space-y-1">
          ...
          <tbody>
            {categorias.map((c) => (
              <tr key={c.id} className="border-t hover:bg-muted/30">
                ...
                <td className="p-3">
                  <button onClick={() => handleExcluir(c.id, c.nome)} className="text-destructive hover:opacity-70 transition">
